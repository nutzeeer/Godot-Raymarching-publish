...
class_name RefractiveModifier24
...
func get_forloop_modifier_template() -> String:
	...
	if (r < current_accuracy) {
		...
		t += current_accuracy * 2.0 + sign(d);  // Step a bit more to get inside
		if (r < 0.0){
			t -= d;
		}
		current_accuracy = t * SURFACE_DISTANCE * pixel_scale;
		ray_origin = pos;
		pos = ray_origin + current_rd * t; // early recalculation
		t = 0.0; //t reset for next iteration
		continue;
	}
	"""
