...
class_name ReflectionModifier5
...
func get_forloop_modifier_template() -> String:
	...
	// Check for reflection
		...
		vec3 reflection_ro = pos + N * current_accuracy * 2.0;
		float rt = current_accuracy * 2.1;
		...
		// Mix current albedo with metallic color and reflection
		ALBEDO = mix(ALBEDO, {metallic_color}, 1.0- fresnel);
		
		hit = true;
		hit_pos = prev_pos;
		hit_normal = getNormal(prev_pos);
...
func get_color_modifier_template() -> String:
	return """
	// Modify color based on reflection and metallic properties
	float NdotV = max(dot(hit_normal, normalize(ray_origin - hit_pos)), 0.0);
	float fresnel = 0.04 + (1.0 - 0.04) * pow(1.0 - NdotV, {fresnel_power});
	vec3 reflection_color = hit_normal * 0.5 + 0.5;
	vec3 metal_reflection = mix({metallic_color}, reflection_color, fresnel);
	
	ALBEDO = mix(ALBEDO, metal_reflection, fresnel);
	"""

func get_custom_map_name() -> String:
	return "map_reflective3"
