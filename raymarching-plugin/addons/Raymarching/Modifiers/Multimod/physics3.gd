...
class_name PhysicsFractalModifier3
...
func get_pre_map_functions() -> String:
	return """
	vec3 calculate_motion(vec3 p, float physics_t, float g) {
		return vec3(0.0, -0.5 * g * physics_t * physics_t, 0.0);
	}
	
	vec3 get_velocity(float physics_t, float g) {
		return vec3(0.0, -g * physics_t, 0.0);
	}
...
func get_p_modifier_template() -> String:
	return """
	// Calculate analytical motion using physics time
	vec3 motion = calculate_motion(p, PHYSICS_TIME, {gravity_strength});
	vec3 current_velocity = get_velocity(PHYSICS_TIME, {gravity_strength});
	
	// Apply motion to transform space
	...
	vec3 fractal_deform = get_velocity_aligned_fractal(
		...
		{fractal_scale},
		{turbulence},
		TIME  // Keep real time for fractal animation
	);
