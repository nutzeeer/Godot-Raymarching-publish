...
float sdf_shape%s_plane2d(vec3 p, vec2 size, float curvature, float thickness, float roundness) {
	...
	// Calculate curved surface with sign-aware distance
	float curve_height = curvature * (p.x * p.x);
	float mid = curve_height;
	float half_t = thickness * 0.5;
	
	// Handle both concave and convex cases
	float upper = p.y - (mid + half_t);
	float lower = (mid - half_t) - p.y;
	
	// Use smooth minimum for concave case
	float height_dist = (curvature >= 0.0) ? 
		max(upper, lower) :  // convex case
		min(upper, lower);   // concave case
	
	float dist = max(plane_dist, height_dist);
